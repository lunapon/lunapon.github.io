...
function arrayToProduct(id, sheetData, sheet) {
  ...
  for (var i = 1; i < sheetData.length; i++) {
    var name = sheetData[(i - 1), 1];
    var image = sheetData[(i - 1), 2];
    var link = sheetData[(i - 1), 3];
    var lowprice = sheetData[(i - 1), 4].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    var highprice = sheetData[(i - 1), 5].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    var tag = sheetData[(i - 1), 6];
    //HTML
