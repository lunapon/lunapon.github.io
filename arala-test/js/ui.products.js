...
function arrayToProduct(id, sheetData, sheet) {
  ...
  for (var i = 1; i < sheetData.length; i++) {
    ...
    var image = sheetData[i][1];
    var link = sheetData[i][2];
    var lowprice = sheetData[i][3].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    var highprice = sheetData[i][4].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    var tag = sheetData[i][5];
    //HTML
