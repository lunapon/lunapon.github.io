...
function arrayToProduct(id, sheetData, sheet) {
  ...
  for (var i = 1; i < sheetData.length; i++) {
    var name = sheetData[1, (i - 1)];
    var image = sheetData[2, (i - 1)];
    var link = sheetData[3, (i - 1)];
    var lowprice = sheetData[4, (i - 1)].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    var highprice = sheetData[5, (i - 1)].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    var tag = sheetData[6, (i - 1)];
    //HTML
